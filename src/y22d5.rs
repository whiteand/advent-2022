...
fn apply_move_one_by_one(m: Move, stacks: &mut Vec<Vec<char>>) {
    for _ in 0..m.amount {
...
fn solve(file_content: &str, apply_move: impl Fn(Move, &mut Vec<Vec<char>>)) -> String {
...
fn apply_move_with_multiple_at_once(m: Move, stacks: &mut Vec<Vec<char>>) {
    if m.from == m.to {
        return;
    }
